...
/*   Updated: 2018/10/29 14:14:09 by vde-sain    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static int			ft_fill_line(char *str, char **line, t_gnlist *list)
{
	int				i;
	char			*tmp;
	int				new_line;
...
static t_gnlist		*ft_gnlstnew(t_gnlist *list, int fd, t_gnlist *head)
{
...
int					get_next_line(const int fd, char **line)
{
	...
	if (list && list->fd != fd)
	{
		if (!(list = ft_gnlstnew(list, fd, head)))
			return (-1);
	}
	if (ft_fill_str(list) == -1)
		return (-1);
	if ((ft_fill_line(list->str, line, list)) == 1)
		return (1);
	return (0);
}
